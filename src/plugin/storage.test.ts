...
describe("Storage Migration", () => {
  ...
  describe("Rolling Token Buckets", () => {
    const baseTime = 1000000000000;

    it("adds tokens to 15-min and 1-day buckets and computes rolling usage", () => {
      let state = addTokensToUsageState(undefined, "claude", 5000, baseTime);
      expect(state.fiveHourBuckets).toHaveLength(1);
      expect(state.weeklyBuckets).toHaveLength(1);

      state = addTokensToUsageState(state, "claude", 3000, baseTime + 1000);
      expect(state.fiveHourBuckets).toHaveLength(1);
      expect(state.fiveHourBuckets?.[0]?.tokens).toBe(8000);

      const usage5h = getRollingTokenUsage(state, "claude", 5 * 3600 * 1000, baseTime + 1000);
      expect(usage5h).toBe(8000);
    });

    it("evicts buckets older than window bounds", () => {
      let state = addTokensToUsageState(undefined, "claude", 10000, baseTime);
      const after6h = baseTime + 6 * 3600 * 1000;
      state = addTokensToUsageState(state, "claude", 2000, after6h);

      const usage5h = getRollingTokenUsage(state, "claude", 5 * 3600 * 1000, after6h);
      expect(usage5h).toBe(2000);
