...
let lastLoggedContextKey = "";
let lastLoggedContextTime = 0;

export function logAccountContext(label: string, info: AccountDebugInfo): void {
  runWithDebugEnabled(() => {
    const key = `${label}:${info.index}:${info.family}`;
    const now = Date.now();
    if (key === lastLoggedContextKey && now - lastLoggedContextTime < 2000) {
      return;
    }
    lastLoggedContextKey = key;
    lastLoggedContextTime = now;
