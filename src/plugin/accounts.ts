...
import type { QuotaGroup, QuotaGroupSummary, GeminiCliQuotaSummary, GeminiCliQuotaModel } from "./quota.js";
...
function isOverSoftQuotaThreshold(
  ...
  if (headerStyle === "gemini-cli") {
    if (!account.cachedGeminiCliQuota) return false;
    ...
    if (!cliQuota.models || cliQuota.models.length === 0) return false;

    let matchedModel: GeminiCliQuotaModel | undefined;
    if (model) {
    ...
    if (remainingFraction == null) return false;
...
export class AccountManager {
  ...
  getEffective5HourCap(account: ManagedAccount, family: ModelFamily): number {
    ...
    if (family === "claude") return 500_000;
    return 1_500_000;
  }
  ...
  markRateLimitedWithReason(
    ...
    if (reason === "QUOTA_EXHAUSTED") {
      ...
      if (current5h > 0) {
        ...
        const updatedUsageState: AccountTokenUsageState = {
          ...(account.tokenUsage || {}),
          fiveHourTokenCap: {
            ...currentCaps,
            [family]: learnedCap,
  ...
  clearAccountVerificationRequired(accountOrIndex: number | ManagedAccount, enableAccount = false): boolean {
  ...
  getAccountsByTag(tag: string): ManagedAccount[] {
    return this.accounts.filter((a) => a.tag === tag);
  }

  setTagEnabled(tag: string, enabled: boolean): number {
    let count = 0;
    for (const account of this.accounts) {
      if (account.tag === tag) {
  ...
  removeAccountsByTag(tag: string): number {
  ...
  private buildStorage(): AccountStorageV4 {
...
export function clearAccountVerificationRequired(account: ManagedAccount, enableAccount = false): boolean {
  const wasVerificationRequired = account.verificationRequired === true;
  const hadMetadata = (
    account.verificationRequiredAt !== undefined ||
    account.verificationRequiredReason !== undefined ||
