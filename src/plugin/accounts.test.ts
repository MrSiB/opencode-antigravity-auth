...
describe("AccountManager", () => {
  ...
  describe("getMinWaitTimeForSoftQuota", () => {
    ...
    it("stores and evaluates geminiCliQuota in updateQuotaCache and soft quota checks when headerStyle is gemini-cli", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
      ...
      const geminiCliQuota = {
        models: [
          { modelId: "gemini-2.5-pro", remainingFraction: 0.05, resetTime: "2026-01-28T15:00:00Z" },
        ],
      };

      manager.updateQuotaCache(0, {
        "gemini-pro": { remainingFraction: 0.9, resetTime: "2026-01-28T15:00:00Z", modelCount: 1 },
      }, geminiCliQuota);

      const accounts = manager.getAccounts();
      expect(accounts[0]?.cachedGeminiCliQuota).toEqual(geminiCliQuota);

      expect(manager.isAccountOverSoftQuota(accounts[0]!, "gemini", 90, 10 * 60 * 1000, "gemini-2.5-pro", "antigravity")).toBe(false);
      expect(manager.isAccountOverSoftQuota(accounts[0]!, "gemini", 90, 10 * 60 * 1000, "gemini-2.5-pro", "gemini-cli")).toBe(true);
      expect(manager.isAccountOverSoftQuota(accounts[1]!, "gemini", 90, 10 * 60 * 1000, "gemini-2.5-pro", "gemini-cli")).toBe(false);
    });
  });

  describe("account origin tags", () => {
    it("initializes tag property from stored metadata", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", email: "a1@test.com", tag: "remote-team-10.0.0.1", addedAt: 1, lastUsed: 0 },
          { refreshToken: "r2", email: "a2@test.com", addedAt: 1, lastUsed: 0 },
        ],
        activeIndex: 0,
      };
      ...
      const manager = new AccountManager(undefined, stored);
      const accounts = manager.getAccounts();
      expect(accounts[0]?.tag).toBe("remote-team-10.0.0.1");
      expect(accounts[1]?.tag).toBeUndefined();
    });

    it("persists tag across saveToDisk", async () => {
      ...
      expect(saveAccounts).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          accounts: expect.arrayContaining([
            expect.objectContaining({ email: "a1@test.com", tag: "remote-team-10.0.0.1" }),
            expect.objectContaining({ email: "a2@test.com" }),
          ]),
        })
      );

      const savedAccounts = (vi.mocked(saveAccounts).mock.calls[0]?.[0] as AccountStorageV4).accounts;
      expect(savedAccounts[0]?.tag).toBe("remote-team-10.0.0.1");
      expect(savedAccounts[1]).not.toHaveProperty("tag");
    });
  });

  describe("tag-based account filtering and bulk operations", () => {
    it("getAccountsByTag returns accounts matching tag", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0, tag: "team-group-1" },
          { refreshToken: "r2", projectId: "p2", addedAt: 1, lastUsed: 0, tag: "team-group-2" },
          { refreshToken: "r3", projectId: "p3", addedAt: 1, lastUsed: 0, tag: "team-group-1" },
        ],
        activeIndex: 0,
      };

      const manager = new AccountManager(undefined, stored);
      const friend1Accounts = manager.getAccountsByTag("team-group-1");
      expect(friend1Accounts.length).toBe(2);
      expect(friend1Accounts[0]?.parts.refreshToken).toBe("r1");
      expect(friend1Accounts[1]?.parts.refreshToken).toBe("r3");

      expect(manager.getAccountsByTag("nonexistent")).toEqual([]);
    });

    it("setTagEnabled bulk updates enabled state and persists via saveToDisk", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0, tag: "group-a", enabled: true },
          { refreshToken: "r2", projectId: "p2", addedAt: 1, lastUsed: 0, tag: "group-b", enabled: true },
          { refreshToken: "r3", projectId: "p3", addedAt: 1, lastUsed: 0, tag: "group-a", enabled: true },
        ],
        activeIndex: 0,
      };

      const manager = new AccountManager(undefined, stored);
      const count = manager.setTagEnabled("group-a", false);

      expect(count).toBe(2);
      expect(manager.getAccountsByTag("group-a").every((a) => a.enabled === false)).toBe(true);
      expect(manager.getAccountsByTag("group-b")[0]?.enabled).toBe(true);
    });

    it("removeAccountsByTag filters out matching accounts, updates indices, and calls saveAccountsReplace", async () => {
      const saveAccountsReplaceMock = (await import("./storage")).saveAccountsReplace;
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0, tag: "group-a" },
          { refreshToken: "r2", projectId: "p2", addedAt: 1, lastUsed: 0, tag: "group-b" },
          { refreshToken: "r3", projectId: "p3", addedAt: 1, lastUsed: 0, tag: "group-a" },
        ],
        activeIndex: 0,
      };

      const manager = new AccountManager(undefined, stored);
      const count = manager.removeAccountsByTag("group-a");

      expect(count).toBe(2);
      expect(manager.getTotalAccountCount()).toBe(1);
      
      const remaining = manager.getAccounts();
      expect(remaining[0]?.parts.refreshToken).toBe("r2");
      expect(remaining[0]?.index).toBe(0);

      expect(saveAccountsReplaceMock).toHaveBeenCalledWith(
        expect.objectContaining({
          version: 4,
          accounts: expect.arrayContaining([
            expect.objectContaining({ refreshToken: "r2" }),
          ]),
        })
      );
    });
...
describe("Token Limit Tracking & 95% Soft-Cap Rotation", () => {
  it("records token usage and evaluates 95% soft cap", () => {
    const stored: AccountStorageV4 = {
      version: 4,
      accounts: [
        { refreshToken: "r1", addedAt: 1, lastUsed: 0, enabled: true },
        { refreshToken: "r2", addedAt: 2, lastUsed: 0, enabled: true },
      ],
      activeIndex: 0,
    };
    ...
    manager.recordTokenUsage(acc1, "claude", 480_000);
    expect(manager.get5HourRollingTokenUsage(acc1, "claude")).toBe(480_000);
    expect(manager.isAccountTokenSoftCapExhausted(acc1, "claude")).toBe(true);

    const next = manager.getNextForFamily("claude");
    expect(next?.parts.refreshToken).toBe("r2");
  });

  it("learns empirical cap on 429 QUOTA_EXHAUSTED", () => {
    const stored: AccountStorageV4 = {
      version: 4,
      accounts: [
        { refreshToken: "r1", addedAt: 1, lastUsed: 0, enabled: true },
    ...
    manager.recordTokenUsage(acc1, "claude", 300_000);
    manager.markRateLimitedWithReason(acc1, "claude", "antigravity", undefined, "QUOTA_EXHAUSTED");

    expect(acc1.tokenUsage?.fiveHourTokenCap?.claude).toBe(270_000);
  });

  it("does not select account in hybrid mode if rate-limited for requested headerStyle", () => {
    const stored: AccountStorageV4 = {
      version: 4,
      accounts: [
        { refreshToken: "r1", addedAt: 1, lastUsed: 0, enabled: true },
    ...
    manager.markRateLimited(acc1, 3600_000, "gemini", "antigravity", "antigravity-gemini-3.6-flash-high");
