...
describe("cleanJSONSchemaForAntigravity", () => {
  ...
  it("removes unsupported keywords nullable, anyOf, oneOf, allOf", () => {
    const schema = {
      type: "object",
      properties: {
        field1: { type: "string", nullable: true },
        field2: { anyOf: [{ type: "string" }, { type: "number" }] },
        field3: { oneOf: [{ type: "boolean" }] },
        field4: { allOf: [{ type: "string" }] },
      },
    };

    const result = cleanJSONSchemaForAntigravity(schema);

    expect(result.properties.field1.nullable).toBeUndefined();
    expect(result.properties.field2.anyOf).toBeUndefined();
    expect(result.properties.field3.oneOf).toBeUndefined();
    expect(result.properties.field4.allOf).toBeUndefined();
  });
});
