...
export async function fetchProjectID(accessToken: string): Promise<string> {
  ...
  for (const baseEndpoint of loadEndpoints) {
    try {
      ...
      const allowedTiers = data.allowedTiers as Array<{ id?: string; isDefault?: boolean }> | undefined;
      ...
      try {
        ...
        if (onboardResponse.ok) {
          ...
          const onboardProjectId =
            typeof onboardProj === "string" ? onboardProj : onboardProj?.id;
          if (onboardProjectId) {
          ...
          if (recheckResponse.ok) {
            ...
            if (
              recheckData.cloudaicompanionProject &&
        ...
        errors.push(
          `onboardUser error at ${baseEndpoint}: ${
            onboardErr instanceof Error ? onboardErr.message : String(onboardErr)
          }`,
        );
      }
