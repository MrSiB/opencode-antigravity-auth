...
export { ANTIGRAVITY_DEFAULT_PROJECT_ID, ANTIGRAVITY_ENDPOINT, ANTIGRAVITY_ENDPOINT_FALLBACKS, ANTIGRAVITY_ENDPOINT_PROD, ANTIGRAVITY_PROVIDER_ID, getAntigravityHeaders, } from "./constants.js";
...
import { clearAccounts, loadAccounts, removeAccountFromStorage, saveAccounts } from "./plugin/storage.js";
...
import { createAntigravityOnlyModelErrorResponse, extractRequestedGeminiModel, fetchWithAgySdkCredential, fetchGeminiApiModels, getAgySdkCredentials, isAgySdkSupportedRequest, isAntigravityOnlyGenerativeLanguageRequest, isApiKeyAuth, isDummyApiKey, isRetryableAgySdkCredentialStatus, selectAgySdkCredential, } from "./plugin/api-key.js";
...
import { recordPublicGeminiApiModels } from "./plugin/model-catalog.js";
...
async function tryFetchWithAgySdkCredentials(input, init, credentials, fallbackRetryAfterMs) {
    ...
    while (attempted.size < credentials.length) {
        const credential = selectAgySdkCredential(credentials.filter((candidate) => !attempted.has(candidate.apiKey)));
        if (!credential) {
            break;
        }
...
async function tryAgySdkFallbackForRequest(input, init, config, credentials, urlString) {
    ...
    if (typeof input !== "string" && "bodyUsed" in input && input.bodyUsed) {
        log.warn("agy-sdk fallback skipped: original request body already consumed", {
            url: sanitizeUrlForLog(urlString),
        });
        return null;
    }
    return tryFetchWithAgySdkCredentials(input, init, validCredentials, defaultRetryMsForConfig(config));
}
...
function remapAccountStateAfterRemoval(removedIndex) {
    ...
    for (const [index, state] of failureEntries) {
        const next = remapIndexAfterRemoval(index, removedIndex);
        if (next !== null)
            accountFailureState.set(next, state);
    }
    ...
    for (const [key, state] of rateEntries) {
        ...
        const next = remapIndexAfterRemoval(index, removedIndex);
        if (next !== null)
            rateLimitStateByAccountQuota.set(`${next}:${quotaKey}`, state);
    }
...
export const loopEscapeTestHooks = {
    ...
    getRateLimitConsecutive(index, quotaKey) {
...
export const createAntigravityPlugin = (providerId) => async ({ client, directory }) => {
    ...
    if (config.health_score) {
        ...
        if (existingTracker && existingTracker.config) {
            Object.assign(existingTracker.config, config.health_score);
        }
        else {
            initHealthTracker({
    ...
    if (config.token_bucket) {
        ...
        if (existingTokenTracker && existingTokenTracker.config) {
            Object.assign(existingTokenTracker.config, config.token_bucket);
        }
        else {
            initTokenTracker({
                ...
                return {
                    ...
                    async fetch(input, init) {
                        ...
                        if (accountManager.getAccountCount() === 0) {
                            ...
                            return createSyntheticErrorResponse("No Antigravity accounts available and no valid Gemini API key configured. Run `opencode auth login` to authenticate.", requestedModel, family);
                        }
                        const urlString = toUrlString(input);
                        const family = getModelFamilyFromUrl(urlString);
                        ...
                        while (true) {
                            ...
                            if (++loopGuard > Math.max(10, accountCount * 2)) {
                                ...
                                const errorMessage = `[Antigravity Error] Request routing did not converge for ${model ?? family}.\n` +
                            ...
                            if (!account) {
                                ...
                                if (accountCount > 0 && triedSwitchIndices.size >= accountCount) {
                                    ...
                                    if (lastFailure) {
                                        return transformAntigravityResponse(lastFailure.response, lastFailure.streaming, lastFailure.debugContext, lastFailure.requestedModel, lastFailure.projectId, lastFailure.endpoint, lastFailure.effectiveModel, lastFailure.sessionId, lastFailure.toolDebugMissing, lastFailure.toolDebugSummary, lastFailure.toolDebugPayload, debugLines, lastFailure.requestPayload, lastFailure.accountEmail);
                                    }
                                    const errorMessage = `[Antigravity Error] All ${accountCount} Antigravity account(s) are rate-limited for ${model ?? family}.\n` +
                                        "Run `opencode auth login` to add accounts or wait for quota reset.";
                                    return createSyntheticErrorResponse(errorMessage, model || undefined, family);
                                }
                                if (accountManager.areAllAccountsOverSoftQuota(family, config.soft_quota_threshold_percent, softQuotaCacheTtlMs, model, preferredHeaderStyle)) {
                                    const threshold = config.soft_quota_threshold_percent;
                                    const softQuotaWaitMs = accountManager.getMinWaitTimeForSoftQuota(family, threshold, softQuotaCacheTtlMs, model, preferredHeaderStyle);
                            ...
                            const runThinkingWarmup = async (prepared, projectId) => {
                                ...
                                try {
                                    ...
                                    const transformed = await transformAntigravityResponse(warmupResponse, true, warmupDebugContext, prepared.requestedModel, projectId, warmupUrl, prepared.effectiveModel, prepared.sessionId, undefined, undefined, undefined, undefined, prepared.requestPayload, account.email);
                                    ...
                                    try {
                                            ...
                                            if (rateLimitReason === "MODEL_CAPACITY_EXHAUSTED" || rateLimitReason === "SERVER_ERROR") {
                                                ...
                                                if (family === "gemini" && allowQuotaFallback) {
                                                    ...
                                                    if (fallbackStyle) {
                                                        ...
                                                        await showToast(`Server busy on ${currentQuotaName} quota for ${safeModelName}. Trying alternate quota...`, "warning");
                                                        headerStyle = fallbackStyle;
                                                ...
                                                const capacityAgySdkFallback = await tryAgySdkFallbackForRequest(input, init, config, agySdkCredentials, urlString);
                                                ...
                                                pushDebug(`No further usable endpoint or quota for headerStyle=${headerStyle} after capacity exhaustion; switching account`);
                                                lastError = new Error(`Model capacity exhausted (${rateLimitReason}) on account ${account.index} after ${capacityRetryCount} retries`);
                                        ...
                                        if (response.status === 401) {
                                            ...
                                            if (!tokenRefreshedOn401) {
                                                ...
                                                try {
                                                    const refreshed = await refreshAccessToken(accountManager.toAuthDetails(account), client, providerId);
                                                    ...
                                                    if (error instanceof AntigravityTokenRefreshError && error.code === "invalid_grant") {
                                                        ...
                                                        await showToast(`⚠ Account ${accountLabel} authentication revoked (invalid_grant). Switching account...`, "warning");
                                                        if (accountManager.getTotalAccountCount() === 0) {
                                                            if (initialAuthWasOAuth) {
                                                                try {
                                                                    await client.auth.set({
                                            ...
                                            getHealthTracker().recordFailure(account.index);
                                            if (accountCount > 1) {
                                                await showToast(`⚠ Account ${accountLabel} authentication expired (401). Switching account...`, "warning");
                                            }
                                            lastFailure = createFailureContext(response);
                                            shouldSwitchAccount = true;
                                            break;
                                        ...
                                        if ((response.status === 404 || permissionDeniedOnProject) &&
                                            isAgySdkSupportedRequest(urlString)) {
                                            const fallbackModelLabel = extractRequestedGeminiModel(urlString) ?? model ?? family;
                                            const modelUnavailableFallback = await tryAgySdkFallbackForRequest(input, init, config, agySdkCredentials, urlString);
                                            if (modelUnavailableFallback) {
                                                ...
                                                pushDebug(`agy-sdk fallback failed (${modelUnavailableFallback.status}) after Antigravity ${response.status} for ${fallbackModelLabel}`);
                                            ...
                                            pushDebug(`no agy-sdk fallback available for ${fallbackModelLabel} after Antigravity ${response.status}`);
                                        }
                            ...
                            if (lastFailure) {
                                ...
                                if (failureStatus === 400 || failureStatus === 401) {
                                    ...
                                    if (errorBodyText.includes("API_KEY_INVALID") || errorBodyText.includes("API key not valid")) {
                                        ...
                                        return createSyntheticErrorResponse(`All ${accountCount} Antigravity account(s) are rate-limited or exhausted for ${requestedModel}. Run \`opencode auth login\` to add accounts or wait for quota reset.`, requestedModel, family);
                                    }
                                }
                                return transformAntigravityResponse(lastFailure.response, lastFailure.streaming, lastFailure.debugContext, lastFailure.requestedModel, lastFailure.projectId, lastFailure.endpoint, lastFailure.effectiveModel, lastFailure.sessionId, lastFailure.toolDebugMissing, lastFailure.toolDebugSummary, lastFailure.toolDebugPayload, debugLines, lastFailure.requestPayload, lastFailure.accountEmail);
                            }
