...
import { buildThinkingWarmupBody, isGenerativeLanguageRequest, prepareAntigravityRequest, reportTokenUsageTelemetry, transformAntigravityResponse, } from "./plugin/request.js";
...
if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
    process.env.GOOGLE_GENERATIVE_AI_API_KEY = "antigravity-dummy-key";
}
...
let activePluginFetchHandler = null;
let embeddedProxyServer = null;
async function ensureActivePluginFetchHandler() {
    if (activePluginFetchHandler) {
        return activePluginFetchHandler;
    }
    try {
        const dummyGetAuth = async () => {
            const stored = await loadAccounts();
            if (stored && stored.accounts.length > 0) {
                const first = stored.accounts[stored.activeIndex ?? 0] || stored.accounts[0];
                if (first?.refreshToken) {
                    return {
                        type: "oauth",
                        refresh: `${first.refreshToken}|${first.projectId || ""}`,
                    };
                }
            }
            return { type: "oauth", refresh: "" };
        };
        const pluginInit = createAntigravityPlugin(ANTIGRAVITY_PROVIDER_ID);
        const pluginObj = await pluginInit({ client: {}, directory: process.cwd() });
        if (pluginObj.auth?.loader) {
            await pluginObj.auth.loader(dummyGetAuth, { models: {} });
        }
    }
    catch (err) {
        console.error("[antigravity-proxy] Auto-initialization of fetch handler failed:", err);
    }
    return activePluginFetchHandler;
}
export function startEmbeddedProxyServer(port = 51128) {
    if (embeddedProxyServer && embeddedProxyServer.listening) {
    ...
    const server = createHttpServer(async (req, res) => {
        try {
            ...
            let retries = 0;
            while (!activePluginFetchHandler && retries < 50) {
            ...
            const transformedRes = await activePluginFetchHandler(urlStr, requestInit);
            const responseHeaders = {};
            transformedRes.headers.forEach((value, key) => {
                responseHeaders[key] = value;
            });
            res.writeHead(transformedRes.status, responseHeaders);
            if (transformedRes.body) {
                Readable.fromWeb(transformedRes.body).pipe(res);
            }
            else {
                res.end();
            }
    ...
    server.on("error", (err) => {
        ...
        log.error("Embedded proxy server error", err);
    });
    server.listen(port, "127.0.0.1", () => {
        log.debug(`Embedded proxy server running on http://127.0.0.1:${port}`);
    });
    embeddedProxyServer = server;
    return server;
}
...
const SWITCH_ACCOUNT_DELAY_MS = 0; // 0s - delay before switching to another account
/**
 ...
 */
export const createAntigravityPlugin = (providerId) => async (ctx) => {
    try {
        startEmbeddedProxyServer(51128);
    }
    catch (err) {
        log.error("Failed to start embedded proxy server", { error: String(err) });
    }
            ...
            loader: async (getAuth, provider) => {
                ...
                if (!isOAuthAuth(auth)) {
                    try {
                        ...
                        if (storedAccounts && storedAccounts.accounts.length > 0) {
                            const firstAccount = storedAccounts.accounts[storedAccounts.activeIndex ?? 0] || storedAccounts.accounts[0];
                            if (firstAccount?.refreshToken) {
                                auth = {
                                    type: "oauth",
                                    refresh: `${firstAccount.refreshToken}|${firstAccount.projectId || ""}`,
                                };
                            }
                        }
                    }
                ...
                const handler = async (input, init) => {
                    ...
                    while (true) {
                        ...
                        if (loopAttempts > Math.max(accountCount * 2, 20)) {
                            pushDebug(`circuit-breaker: excessive account rotation attempts (${loopAttempts}), sleeping 1s`);
                        ...
                        if (!account) {
                            if (accountManager.areAllAccountsOverSoftQuota(family, config.soft_quota_threshold_percent, softQuotaCacheTtlMs, model)) {
                                ...
                                await sleep(Math.max(softQuotaWaitMs, 1000), abortSignal);
                                continue;
                            }
                            ...
                            await sleep(Math.max(waitMs, 1000), abortSignal);
                            continue;
                        }
                        ...
                        if (accessTokenExpired(authRecord)) {
                            try {
                                ...
                                if (!refreshed) {
                                    const { failures, cooldownMs } = trackAccountFailure(account.index);
                                    getHealthTracker().recordFailure(account.index);
                                    lastError = new Error("Antigravity token refresh failed");
                                    const effectiveCooldown = Math.max(cooldownMs || 0, 30_000);
                                    accountManager.markAccountCoolingDown(account, effectiveCooldown, "auth-failure");
                                    accountManager.markRateLimited(account, effectiveCooldown, family, "antigravity", model);
                                    pushDebug(`token-refresh-failed: cooldown ${effectiveCooldown}ms after ${failures} failures`);
                                    continue;
                                }
                                ...
                                accountManager.markAccountCoolingDown(account, effectiveCooldown, "auth-failure");
                                accountManager.markRateLimited(account, effectiveCooldown, family, "antigravity", model);
                                pushDebug(`token-refresh-error: cooldown ${effectiveCooldown}ms after ${failures} failures`);
                                continue;
                            }
                        }
                        ...
                        if (!accessToken) {
                            ...
                            accountManager.markAccountCoolingDown(account, 30_000, "auth-failure");
                            accountManager.markRateLimited(account, 30_000, family, "antigravity", model);
                            continue;
                        }
                        let projectContext;
                        try {
                ...
                activePluginFetchHandler = handler;
                return {
                        ...
                        if (inputs) {
                            ...
                            while (accounts.length < MAX_OAUTH_ACCOUNTS) {
                                ...
                                if (result.type === "failed") {
                                    ...
                                    console.warn(`[@mrsib/opencode-antigravity-auth] Skipping failed account ${accounts.length + 1}: ${result.error}`);
                                    break;
                                }
