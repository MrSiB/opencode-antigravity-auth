...
function isOverSoftQuotaThreshold(account, family, thresholdPercent, cacheTtlMs, model, headerStyle) {
    if (thresholdPercent >= 100)
        return false;
    if (headerStyle === "gemini-cli") {
        if (!account.cachedGeminiCliQuota)
            return false;
        ...
        if (age > cacheTtlMs)
            return false;
        const cliQuota = account.cachedGeminiCliQuota;
        if (!cliQuota.models || cliQuota.models.length === 0)
            return false;
        let matchedModel;
        if (model) {
            const lowerModel = model.toLowerCase();
            matchedModel = cliQuota.models.find((m) => {
                const lowerId = m.modelId.toLowerCase();
                return lowerModel.includes(lowerId) || lowerId.includes(lowerModel);
            });
        }
        let remainingFraction;
        if (matchedModel) {
            remainingFraction = matchedModel.remainingFraction;
        }
        else {
            const fractions = cliQuota.models
                .map((m) => m.remainingFraction)
                .filter((f) => typeof f === "number");
            if (fractions.length > 0) {
                remainingFraction = Math.min(...fractions);
            }
        }
        if (remainingFraction == null)
            return false;
        const normalizedRemaining = Math.max(0, Math.min(1, remainingFraction));
        const usedPercent = (1 - normalizedRemaining) * 100;
...
export class AccountManager {
        ...
        if (stored && stored.accounts.length > 0) {
            ...
            this.accounts = stored.accounts
                ...
                return {
                    ...
                    cachedGeminiCliQuota: acc.cachedGeminiCliQuota,
    ...
    getNextForFamily(family, model, headerStyle = "antigravity", softQuotaThresholdPercent = 100, softQuotaCacheTtlMs = 10 * 60 * 1000) {
        const activeNonLimited = this.accounts.filter((a) => {
        ...
        if (activeNonLimited.length === 0) {
            return null;
        }
        const unexhausted = activeNonLimited.filter((a) => !this.isAccountTokenSoftCapExhausted(a, family));
        const pool = unexhausted.length > 0 ? unexhausted : activeNonLimited;
    ...
    getEffective5HourCap(account, family) {
        if (account.tokenUsage?.fiveHourTokenCap?.[family]) {
            return account.tokenUsage.fiveHourTokenCap[family];
        }
        if (family === "claude")
            return 500_000;
        return 1_500_000;
    }
    isAccountTokenSoftCapExhausted(account, family) {
        const used = this.get5HourRollingTokenUsage(account, family);
        const cap = this.getEffective5HourCap(account, family);
        return used >= 0.95 * cap;
    }
    markRateLimitedWithReason(account, family, headerStyle, model, reason, retryAfterMs, failureTtlMs = 3600_000) {
        ...
        if (reason === "QUOTA_EXHAUSTED") {
            const current5h = this.get5HourRollingTokenUsage(account, family);
            if (current5h > 0) {
                const learnedCap = Math.max(100_000, Math.floor(0.90 * current5h));
                const currentCaps = account.tokenUsage?.fiveHourTokenCap || {};
                const currentLast429 = account.tokenUsage?.last429Ts || {};
                const updatedUsageState = {
                    ...(account.tokenUsage || {}),
                    fiveHourTokenCap: {
                        ...currentCaps,
                        [family]: learnedCap,
                ...
                account.tokenUsage = updatedUsageState;
                const accountLabel = formatAccountLabel(account.email, account.index);
                log.info(`[AccountManager] Learned empirical 5h cap for ${accountLabel} (${family}): ${learnedCap} tokens`);
            }
        }
    ...
    markRequestSuccess(account) {
        ...
        if (account.verificationRequired) {
            this.clearAccountVerificationRequired(account, true);
        }
    }
    ...
    clearAccountVerificationRequired(accountOrIndex, enableAccount = false) {
        ...
        account.verificationRequiredReason = undefined;
        account.verificationUrl = undefined;
        const accountIndex = typeof accountOrIndex === "number" ? accountOrIndex : this.accounts.indexOf(account);
     ...
     */
    invalidateAccountProjectContext(accountOrIndex) {
        const account = typeof accountOrIndex === "number" ? this.accounts[accountOrIndex] : accountOrIndex;
        if (!account)
            return;
     ...
     */
    async ensureAccountProjectContext(accountOrIndex) {
        const account = typeof accountOrIndex === "number" ? this.accounts[accountOrIndex] : accountOrIndex;
        if (!account)
            return null;
    ...
    getAccountsByTag(tag) {
        return this.accounts.filter((a) => a.tag === tag);
    }
    setTagEnabled(tag, enabled) {
        ...
        if (!enabled) {
            for (const family of Object.keys(this.currentAccountIndexByFamily)) {
                const currentIdx = this.currentAccountIndexByFamily[family];
                if (currentIdx !== undefined && currentIdx >= 0 && currentIdx < this.accounts.length) {
                    const currentAccount = this.accounts[currentIdx];
                    if (currentAccount && currentAccount.enabled === false) {
                        const next = this.accounts.find((a) => a.enabled !== false);
                        this.currentAccountIndexByFamily[family] = next?.index ?? -1;
                    }
                }
            }
    ...
    removeAccountsByTag(tag) {
        const toRemove = this.accounts.filter((a) => a.tag === tag);
        if (toRemove.length === 0) {
            return 0;
        }
        const activeClaude = this.getCurrentAccountForFamily("claude");
        const activeGemini = this.getCurrentAccountForFamily("gemini");
        this.accounts = this.accounts.filter((a) => a.tag !== tag);
        this.accounts.forEach((acc, index) => {
            acc.index = index;
        });
        if (this.accounts.length === 0) {
            ...
            this.cursor = this.cursor % this.accounts.length;
            if (activeClaude && this.accounts.includes(activeClaude)) {
                this.currentAccountIndexByFamily.claude = activeClaude.index;
            }
            else {
                const nextClaude = this.accounts.find((a) => a.enabled !== false);
                this.currentAccountIndexByFamily.claude = nextClaude ? nextClaude.index : -1;
            }
            if (activeGemini && this.accounts.includes(activeGemini)) {
                this.currentAccountIndexByFamily.gemini = activeGemini.index;
            }
            else {
                const nextGemini = this.accounts.find((a) => a.enabled !== false);
                this.currentAccountIndexByFamily.gemini = nextGemini ? nextGemini.index : -1;
            }
        }
        void saveAccountsReplace(this.buildStorage());
    ...
    buildStorage() {
        ...
        return {
            ...
            accounts: this.accounts.map((a) => ({
                ...
                cachedGeminiCliQuota: a.cachedGeminiCliQuota,
    ...
    async saveToDisk() {
        await saveAccounts(this.buildStorage());
    }
    requestSaveToDisk() {
        if (this.savePending) {
            return;
    ...
    updateQuotaCache(accountIndex, quotaGroups, geminiCliQuota) {
        const account = this.accounts[accountIndex];
        if (account) {
            account.cachedQuota = quotaGroups;
            if (geminiCliQuota !== undefined) {
                account.cachedGeminiCliQuota = geminiCliQuota;
            }
            account.cachedQuotaUpdatedAt = nowMs();
        }
    }
    isAccountOverSoftQuota(account, family, thresholdPercent, cacheTtlMs, model, headerStyle) {
        return isOverSoftQuotaThreshold(account, family, thresholdPercent, cacheTtlMs, model, headerStyle);
    }
    ...
    areAllAccountsOverSoftQuota(family, thresholdPercent, cacheTtlMs, model, headerStyle) {
        if (thresholdPercent >= 100)
            return false;
        const enabled = this.accounts.filter(a => a.enabled !== false);
        if (enabled.length === 0)
            return false;
        return enabled.every(a => isOverSoftQuotaThreshold(a, family, thresholdPercent, cacheTtlMs, model, headerStyle));
    }
    /**
     ...
     */
    getMinWaitTimeForSoftQuota(family, thresholdPercent, cacheTtlMs, model, headerStyle) {
        if (thresholdPercent >= 100)
            return 0;
        const enabled = this.accounts.filter(a => a.enabled !== false);
        if (enabled.length === 0)
        ...
        const available = enabled.filter(a => !isOverSoftQuotaThreshold(a, family, thresholdPercent, cacheTtlMs, model, headerStyle));
        if (available.length > 0)
            return 0;
        ...
        if (headerStyle === "gemini-cli") {
            ...
            for (const acc of enabled) {
                const cliQuota = acc.cachedGeminiCliQuota;
                if (!cliQuota?.models)
                    continue;
                let matchedModel;
                if (model) {
                ...
                const modelsToCheck = matchedModel ? [matchedModel] : cliQuota.models;
                for (const m of modelsToCheck) {
                    if (m.resetTime) {
                        const resetTimestamp = Date.parse(m.resetTime);
...
export function clearAccountVerificationRequired(account, enableAccount = false) {
    const wasVerificationRequired = account.verificationRequired === true;
    const hadMetadata = (account.verificationRequiredAt !== undefined ||
        account.verificationRequiredReason !== undefined ||
        account.verificationUrl !== undefined);
    ...
    return wasVerificationRequired || hadMetadata;
}
