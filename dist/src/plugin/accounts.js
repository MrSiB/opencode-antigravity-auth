...
import { loadAccounts, removeAccountFromStorage, saveAccounts, saveAccountsReplace } from "./storage.js";
...
function clearExpiredRateLimits(account) {
    ...
    for (const key of keys) {
        ...
        if (resetTime !== undefined) {
            ...
            const isExpiredByTime = now >= resetTime ||
...
export function extractQuotaMetrics(account, family, model) {
    const now = nowMs();
    if (Array.isArray(account.quotaSummary) && account.quotaSummary.length > 0) {
        const isGemini = family !== "claude";
        const group = account.quotaSummary.find((g) => {
 ...
 */
export function isAccountQuotaExhausted(account, family, model) {
...
export class AccountManager {
    ...
    serializeClearedQuotaKeys(account) {
        ...
        for (const key of Object.keys(account.clearedQuotaKeys)) {
            ...
            const expired = !invalid && now - clearedAt > RATE_LIMIT_CLEAR_TTL_MS;
            const superseded = account.rateLimitResetTimes[key] !== undefined;
    ...
    serializeRateLimitSetTimes(account) {
        ...
        for (const key of Object.keys(account.rateLimitSetTimes)) {
            ...
            const hasLiveLimit = account.rateLimitResetTimes[key] !== undefined;
