...
export function logAntigravityDebugResponse(context, response, meta = {}) {
    ...
    logDebug(`[Antigravity Debug ${context.id}] Response Headers: ${JSON.stringify(maskHeadersForLog(meta.headersOverride ?? response.headers))}`);
    if (meta.note) {
 ...
 */
export function maskHeadersForLog(headers) {
    if (!headers) {
        return {};
    }
    const result = {};
