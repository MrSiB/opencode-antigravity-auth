...
export function selectHybridAccount(accounts, tokenTracker, currentAccountIndex = null, minHealthScore = 50) {
    ...
    const candidates = accounts
        .filter(acc => {
        if (acc.isRateLimited || acc.isCoolingDown)
            return false;
        if (acc.healthScore < minHealthScore)
            return false;
        if (acc.quota) {
            ...
            if (!tokenTracker.hasTokens(acc.index))
                return false;
        }
        return true;
    })
...
export function calculateHybridScore(account, maxTokens = 50) {
