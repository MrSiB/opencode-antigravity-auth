...
/**
 * Strips quota routing prefixes (e.g. "antigravity-") from model name.
 *
 * Examples:
...
export function cleanModelName(model) {
    return model.replace(QUOTA_PREFIX_REGEX, "");
}
...
function resolveGemini35FlashApiModel(model, level) {
...
function resolveGemini36FlashApiModel(model) {
...
export function resolveModelWithTier(requestedModel, options = {}) {
    ...
    const modelWithoutQuota = cleanModelName(requestedModel);
    ...
    const budgetKey = tier === "minimal" ? "low" : tier;
...
export function resolveModelForHeaderStyle(requestedModel, headerStyle) {
    ...
    if (headerStyle === "antigravity") {
        let transformedModel = cleanModelName(requestedModel)
