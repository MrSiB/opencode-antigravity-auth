...
export function isGeminiPublicOnlyModel(model) {
 ...
 */
export function isGeminiCliSupportedModel(model) {
    if (!model)
        return false;
    const normalized = model.toLowerCase().replace(QUOTA_PREFIX_REGEX, "");
...
export function resolveModelWithTier(requestedModel, options = {}) {
    ...
    const preferGeminiCli = !isAntigravity &&
    ...
    if (isEffectiveGemini3) {
        const effectiveTier = tier === "minimal" && (resolvedModel.includes("gemini-3.7") || resolvedModel.includes("gemini-3.8"))
            ? "low"
            : tier;
        return {
            actualModel: resolvedModel,
