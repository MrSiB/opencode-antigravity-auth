...
function nullableUnionSchema(schema) {
    for (const key of ["anyOf", "oneOf"]) {
        ...
        const baseDescription = typeof rest.description === "string"
            ? rest.description
...
function singleOptionUnionSchema(schema) {
    for (const key of ["anyOf", "oneOf"]) {
        ...
        const description = typeof rest.description === "string"
            ? rest.description
...
const STRICT_SAMPLING_MODEL_REGEX = /^gemini-(?:3\.[678]-flash(?:-(?:minimal|low|medium|high|tiered))?|3\.5-flash-lite)$/i;
 ...
 */
export function sanitizeGeminiGenerationConfigForModel(payload, model) {
            ...
            if (Array.isArray(tool.functionDeclarations)) {
                for (const decl of tool.functionDeclarations) {
                    const parameters = decl.parameters &&
                        ...
                        ? toGeminiSchema(decl.parameters)
