...
export function toGeminiSchema(schema) {
    ...
    try {
        cleanJSONSchemaForAntigravity(JSON.parse(JSON.stringify(schema)));
    }
    catch {
    }
...
export function normalizeGeminiTools(payload) {
    ...
    payload.tools = payload.tools.map((tool, toolIndex) => {
        ...
        if (Array.isArray(newTool.functionDeclarations)) {
            ...
            const normalizedDeclarations = newTool.functionDeclarations.map((declaration) => {
                const decl = declaration && typeof declaration === "object" && !Array.isArray(declaration)
                    ? declaration
                    : {};
                const schemaCandidate = decl.parameters ||
                    decl.parametersJsonSchema ||
                    decl.input_schema ||
                    decl.inputSchema;
                const hasSchema = schemaCandidate &&
            ...
            toolDebugSummaries.push(`idx=${toolIndex}, functionDeclarations=${normalizedDeclarations.length}, missingSchemas=${missingDeclarationSchemas}`);
            return newTool;
        }
        ...
        const rawNameCandidate = String(newTool.name ||
            newTool.function?.name ||
            newTool.custom?.name ||
            `tool-${toolIndex}`);
        const nameCandidate = rawNameCandidate.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 64);
        ...
        if (tool.functionDeclarations) {
            if (Array.isArray(tool.functionDeclarations)) {
                for (const decl of tool.functionDeclarations) {
                    const rawDeclName = String(decl.name || `tool-${functionDeclarations.length}`);
                    const formattedDeclName = rawDeclName.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 64);
                    functionDeclarations.push({
                        name: formattedDeclName,
