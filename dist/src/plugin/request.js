...
import { isGemini3Model, isImageGenerationModel, buildImageGenerationConfig, applyGeminiTransforms, sanitizeGeminiGenerationConfigForModel, } from "./transform/index.js";
import { resolveModelForHeaderStyle, resolveAntigravityGemini35FlashBackendModel, resolveAntigravityGemini36FlashBackendModel, resolveAntigravityGemini37FlashBackendModel, resolveAntigravityGemini38FlashBackendModel, getDefaultGemini3ThinkingLevel, isClaudeModel, isClaudeThinkingModel, CLAUDE_THINKING_MAX_OUTPUT_TOKENS, } from "./transform/index.js";
...
export function sanitizeAccountEmail(email) {
...
export function reportTokenUsageTelemetry(requestPayload, model, usage, statsFilePath, accountEmail, projectId, sessionId) {
    if (!usage) {
        return null;
    }
    try {
        const persona = extractAgentPersona(requestPayload);
        const targetPath = statsFilePath ?? join(getConfigDir(), "antigravity-usage-stats.json");
        ...
        const payloadEmail = typeof requestPayload === "object" && requestPayload !== null
            ? requestPayload._accountEmail
            : undefined;
...
const sessionDisplayedThinkingHashes = new Map();
...
function ensureThinkingBeforeToolUseInMessages(messages, signatureSessionKey) {
    return messages.map((message) => {
        ...
        const otherBlocks = blocks.filter((b) => !(b &&
            typeof b === "object" &&
            (b.type === "thinking" || b.type === "redacted_thinking")));
...
function applyClaudeThinkingSanitization(target, opts) {
    const { effectiveModel, signatureSessionKey, isClaudeThinking, keepThinkingEnabled, enableClaudePromptAutoCaching, } = opts;
    ...
    deepFilterThinkingBlocks(target, signatureSessionKey, getCachedSignature, true);
    if (enableClaudePromptAutoCaching &&
    ...
    if (isClaudeThinking &&
        ...
        target.contents = ensureThinkingBeforeToolUseInContents(target.contents, signatureSessionKey);
    }
    if (isClaudeThinking &&
        keepThinkingEnabled &&
        Array.isArray(target.messages)) {
        target.messages = ensureThinkingBeforeToolUseInMessages(target.messages, signatureSessionKey);
    }
}
/**
...
export function isGenerativeLanguageRequest(input) {
    try {
        return (new URL(requestInfoUrl(input)).hostname ===
        ...
        try {
            ...
            if (isWrapped) {
                ...
                if (headerStyle === "antigravity") {
                    const gemini38FlashBackendModel = resolveAntigravityGemini38FlashBackendModel(effectiveModel, tierThinkingLevel);
                    const gemini37FlashBackendModel = resolveAntigravityGemini37FlashBackendModel(effectiveModel, tierThinkingLevel);
                    const gemini36FlashBackendModel = resolveAntigravityGemini36FlashBackendModel(effectiveModel, tierThinkingLevel);
                    const gemini35FlashBackendModel = resolveAntigravityGemini35FlashBackendModel(effectiveModel, tierThinkingLevel);
                ...
                for (const req of requestObjects) {
                    ...
                    sanitizeGeminiGenerationConfigForModel(req, effectiveModel);
                ...
                const effectiveUserThinkingConfig = isClaudeSonnetNonThinking || isImageModel
                    ? undefined
                    : userThinkingConfig;
                ...
                if (isImageModel) {
                    ...
                    const finalThinkingConfig = resolveThinkingConfig(effectiveUserThinkingConfig, isClaudeSonnetNonThinking
                        ? false
                ...
                const hasTools = Array.isArray(requestPayload.tools) &&
                    requestPayload.tools.length > 0;
                if (hasTools) {
                    if (isClaude) {
                ...
                sanitizeGeminiGenerationConfigForModel(requestPayload, effectiveModel);
 ...
 */
export async function transformAntigravityResponse(response, streaming, debugContext, requestedModel, projectId, endpoint, effectiveModel, sessionId, toolDebugMissing, toolDebugSummary, toolDebugPayload, debugLines, requestPayload, accountEmail) {
        ...
        const streamingTransformer = createStreamingTransformer(defaultSignatureStore, {
            ...
            onUsageMetadata: (rawUsage) => {
                if (rawUsage && effectiveModel) {
                    ...
                    if (usage) {
                        reportTokenUsageTelemetry(requestPayload, effectiveModel, usage, undefined, accountEmail, projectId, sessionId);
                    }
                }
            },
        }, {
