 ...
 */
export function buildLocalStatusData(accountManager) {
    if (!accountManager) {
        return {
            accountCount: 0,
    ...
    let total7dClaude = 0;
    let total7dGemini = 0;
    const empiricalCapacitiesClaude = {};
    const empiricalCapacitiesGemini = {};
    const accounts = rawAccounts.map((acc) => {
        ...
        if (capClaude !== undefined)
            empiricalCapacitiesClaude[acc.index] = capClaude;
        if (capGemini !== undefined)
            empiricalCapacitiesGemini[acc.index] = capGemini;
        return {
            index: acc.index,
            email: masked,
 ...
 */
export function renderDashboardHtml(statusData) {
