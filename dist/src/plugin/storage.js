...
export class AccountStorageCorruptedError extends Error {
    path;
    cause;
    constructor(message, pathOrCause, cause) {
        let extractedPath;
        let extractedCause;
...
export function parseDeletedHashEntry(entry) {
    ...
    if (typeof entry === "object" && entry !== null) {
        ...
        if (typeof obj.hash === "string" && obj.hash.trim()) {
            const ts = typeof obj.deletedAt === "number"
                ? obj.deletedAt
...
export function getActiveDeletedHashes(entries, now = Date.now()) {
...
export function serializeDeletedHashes(hashes) {
    if (hashes.size === 0)
        return undefined;
    const list = [];
...
function activeClearMarkers(source, setGenerations, now) {
    ...
    for (const [key, value] of Object.entries(source)) {
        ...
        if (clearedAt === undefined)
            continue;
        if (now - clearedAt > RATE_LIMIT_CLEAR_TTL_MS)
            continue;
...
function mergeAccountStorage(existing, incoming) {
    ...
    const activeDeletedHashes = new Map();
    ...
    return {
        ...
        activeIndexByFamily: remapActiveIndexByFamily(incoming.activeIndexByFamily, incoming.accounts, accounts),
...
function remapActiveIndex(accounts, deduplicatedAccounts, activeIndex) {
    ...
    const remappedIndex = deduplicatedAccounts.findIndex((account) => accountIdentity(account) === selectedIdentity);
...
function remapActiveIndexByFamily(activeIndexByFamily, accounts, deduplicatedAccounts) {
    ...
    return {
        ...(activeIndexByFamily.claude === undefined
            ? {}
            : {
                claude: remapActiveIndex(accounts, deduplicatedAccounts, activeIndexByFamily.claude),
            }),
        ...(activeIndexByFamily.gemini === undefined
            ? {}
            : {
                gemini: remapActiveIndex(accounts, deduplicatedAccounts, activeIndexByFamily.gemini),
            }),
    };
}
...
async function readAndRecoverAccountsFile(path) {
    ...
    const corruptionReason = isZeroByte
        ? "0-byte file"
        : `SyntaxError: ${syntaxError?.message || "invalid JSON"}`;
    ...
    if (restoredData !== null) {
        ...
        log.warn(`Corrupted accounts file detected (${corruptionReason}). Quarantined to ${corruptedPath} and restored from ${bakPath}.`);
        return { data: restoredData, restoredFromBak: true };
    }
    const detailMsg = typeof bakContent !== "string"
...
export async function loadAccounts(path = getStoragePath()) {
    try {
        ...
        const validAccounts = storage.accounts.filter((a) => {
            return (!!a &&
                ...
                !activeDeletedHashes.has(hashRefreshToken(a.refreshToken)));
        });
        ...
        if (deduplicatedAccounts.length > 0 && deduplicatedAccounts.every((acc) => acc.enabled === false)) {
            ...
            const healedStorage = {
                ...
                activeIndex: remapActiveIndex(validAccounts, deduplicatedAccounts, storage.activeIndex),
                deletedRefreshTokenHashes: serializeDeletedHashes(activeDeletedHashes),
                activeIndexByFamily: remapActiveIndexByFamily(storage.activeIndexByFamily, validAccounts, deduplicatedAccounts),
            };
            saveAccounts(healedStorage).catch((err) => {
                log.warn("Failed to persist healed accounts storage", { error: String(err) });
            });
        }
        const activeIndex = remapActiveIndex(validAccounts, deduplicatedAccounts, storage.activeIndex);
        return {
            version: 4,
            accounts: deduplicatedAccounts,
            activeIndex,
...
export async function saveAccountsReplace(storage) {
    ...
    await withFileLock(path, async () => {
        ...
        await writeAccountsAtomically(path, {
            ...storage,
            accounts: storage.accounts.filter((account) => !activeDeletedHashes.has(hashRefreshToken(account.refreshToken))),
            deletedRefreshTokenHashes: serializeDeletedHashes(activeDeletedHashes),
        });
    });
}
export async function removeAccountFromStorage(refreshToken) {
    const path = getStoragePath();
    const configDir = dirname(path);
    await fs.mkdir(configDir, { recursive: true });
    await ensureGitignore(configDir);
    await withFileLock(path, async () => {
        ...
        await writeAccountsAtomically(path, {
            ...
            deletedRefreshTokenHashes: serializeDeletedHashes(activeDeletedHashes),
            activeIndexByFamily: remapActiveIndexByFamily(existing.activeIndexByFamily, existing.accounts, accounts),
        });
    });
}
export const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
export const delay = sleep;
export const RENAME_RETRY_DELAYS_MS = [50, 100, 150, 200, 250];
export async function writeAccountsAtomically(path, storage) {
    try {
    ...
    try {
        ...
        try {
            ...
            const initialCode = renameError?.code;
...
export async function loadAccountsUnsafe(path = getStoragePath()) {
    const result = await readAndRecoverAccountsFile(path);
    if (!result) {
        return null;
    }
    ...
    if (parsed.version === 1) {
        return remapDeduplicatedStorage(migrateV3ToV4(migrateV2ToV3(migrateV1ToV2(parsed))));
    }
    if (parsed.version === 2) {
...
function remapDeduplicatedStorage(storage) {
    ...
    return {
        ...storage,
        accounts: deduplicatedAccounts,
        activeIndex: remapActiveIndex(storage.accounts, deduplicatedAccounts, storage.activeIndex),
        activeIndexByFamily: remapActiveIndexByFamily(storage.activeIndexByFamily, storage.accounts, deduplicatedAccounts),
    };
}
