...
function pruneBuckets(buckets, maxSpanMs, now) {
    const cutoff = now - maxSpanMs;
    return buckets.filter((b) => b.timestamp >= cutoff);
}
export function addTokensToUsageState(state, _family, tokens, now = Date.now()) {
...
export function getRollingTokenUsage(state, _family, windowMs, now = Date.now()) {
    if (!state)
        return 0;
