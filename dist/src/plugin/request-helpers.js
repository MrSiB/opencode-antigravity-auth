...
import { EMPTY_SCHEMA_PLACEHOLDER_NAME, EMPTY_SCHEMA_PLACEHOLDER_DESCRIPTION, SKIP_THOUGHT_SIGNATURE, MIN_SIGNATURE_LENGTH, } from "../constants.js";
...
const schemaCleanCache = new Map();
/**
 ...
 */
export function resolveThinkingConfig(userConfig, isThinkingModel) {
 ...
 */
export function getThinkingText(part) {
...
function collectStringsFrom(val, visited = new WeakSet()) {
...
function extractSystemInstructionText(payload) {
    const targets = [
        payload.systemInstruction,
    ...
    if (Array.isArray(payload.contents)) {
        for (const content of payload.contents) {
            if (content && typeof content === "object" && content.role === "system") {
...
function extractMessagesText(payload) {
    const targets = [];
    if (payload.messages)
        targets.push(payload.messages);
    if (payload.contents)
        targets.push(payload.contents);
    if (payload.parts)
        targets.push(payload.parts);
    return collectStringsFrom(targets).join(" ");
}
export function extractAgentPersona(payload) {
