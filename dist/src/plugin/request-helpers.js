...
const UNSUPPORTED_KEYWORDS = [
    ...
    "propertyNames", "title", "$id", "$comment", "nullable", "anyOf", "oneOf", "allOf",
];
/**
 ...
 */
function normalizeArrayTypesAndItems(schema) {
    if (!schema || typeof schema !== "object") {
        return schema;
    }
    if (Array.isArray(schema)) {
...
export function extractUsageMetadata(body) {
    ...
    const target = (rawObj.response && typeof rawObj.response === "object"
        ? rawObj.response
        : rawObj);
    const usage = (target.usageMetadata || target.usage);
...
export function extractUsageFromSsePayload(payload) {
    ...
    for (const line of lines) {
        ...
        try {
            ...
            if (parsed && typeof parsed === "object") {
                const usage = extractUsageMetadata(parsed);
                if (usage) {
                    return usage;
                }
            }
