...
import { applyGeminiTransforms, isGemini3Model, isImageGenerationModel, mapAntigravityModelToPublicApi, resolveModelForHeaderStyle, sanitizeGeminiGenerationConfigForModel, } from "./transform/index.js";
import { getPublicGeminiApiModelIds } from "./model-catalog.js";
const rateLimitedUntilByKey = new Map();
let cursor = 0;
const GEMINI_MODELS_LIST_TIMEOUT_MS = 10000;
const GEMINI_MODELS_LIST_MAX_PAGES = 20;
export function isRetryableAgySdkCredentialStatus(status) {
...
export function isDummyApiKey(key) {
    if (!key)
        return true;
...
export function getAgySdkCredentials(config, auth) {
    ...
    for (const project of config.agy_sdk.cloud_projects) {
        if (project.enabled === false)
            continue;
        if (!project.api_key?.trim() || isDummyApiKey(project.api_key))
            continue;
        credentials.push({
    ...
    const envCandidates = [
        ...
        .filter((apiKey) => Boolean(apiKey) && !isDummyApiKey(apiKey));
...
function canRouteAsPublicGeminiApiModel(model) {
    ...
    if (getPublicGeminiApiModelIds()?.has(stripped))
        return true;
...
export function isAgySdkSupportedRequest(urlString) {
    ...
    if (!model || !model.toLowerCase().includes("gemini") || isImageGenerationModel(model))
        return false;
