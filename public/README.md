# Antigravity + Gemini CLI OAuth Plugin for Opencode (Custom Fork)
...
- **Dynamic Quota Fallback**: Enhances quota requests (`quota.js`) to cascade through multiple fallback endpoints: `Daily non-sandbox` -> `Daily sandbox` -> `Prod`.
...
| `antigravity-gemini-3.6-flash` | Gemini 3.6 Flash | `minimal`, `low`, `medium`, `high` |
| `antigravity-gemini-3.5-flash` | Gemini 3.5 Flash | `low`, `medium`, `high` |
| `antigravity-gemini-3-flash` | Gemini 3 Flash | `minimal`, `low`, `medium`, `high` |
| `antigravity-claude-sonnet-4-6` | Claude Sonnet 4.6 | — |
| `antigravity-claude-opus-4-6-thinking` | Claude Opus 4.6 Thinking | `low`, `max` |
...
**Use a current model:**
```bash
opencode run "Hello" --model=google/antigravity-gemini-3.6-flash
opencode run "Hello" --model=google/antigravity-claude-opus-4-6-thinking
```
...
### 403 Permission Denied (`your-project-id`)

**Error:**
```
Permission 'cloudaicompanion.companions.generateChat' denied on resource 
'//cloudaicompanion.googleapis.com/projects/your-project-id/locations/global'
```
...
- [Support Matrix](docs/SUPPORT_MATRIX.md): Feature, model, and credential matrix per transport mode
...
- Original repository: [opencode-antigravity-auth](https://github.com/NoeFabris/opencode-antigravity-auth) by [@NoeFabris](https://github.com/NoeFabris)
