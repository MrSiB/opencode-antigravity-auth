...
let activePluginFetchHandler: ((input: RequestInfo, init?: RequestInit) => Promise<Response>) | null = null;
let embeddedProxyServer: HttpServer | null = null;

async function ensureActivePluginFetchHandler(): Promise<((input: RequestInfo, init?: RequestInit) => Promise<Response>) | null> {
  if (activePluginFetchHandler) {
    return activePluginFetchHandler;
  }
  try {
    const dummyGetAuth: GetAuth = async () => {
    ...
    const pluginObj = await pluginInit({ client: {} as any, directory: process.cwd() });
    if (pluginObj.auth?.loader) {
      await pluginObj.auth.loader(dummyGetAuth, { models: {} } as any);
    }
  } catch (err) {
...
export function startEmbeddedProxyServer(port: number = 51128): HttpServer {
  ...
  const server = createHttpServer(async (req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if (transformedRes.body) {
        Readable.fromWeb(transformedRes.body as any).pipe(res);
      } else {
        res.end();
      }
    } catch (err) {
