...
| **Thinking Models & Budget Signatures** | Yes (`minimal`, `low`, `medium`, `high`, `max`) | No | Partial (Model dependent) |
...
| Model Identifier | `gateway` | `cli` | `managed-agent` | Notes / Variants |
|------------------|-----------|-------|-----------------|------------------|
| `antigravity-gemini-3.6-flash` | Yes | No | No | Active Antigravity quota set (`minimal`, `low`, `medium`, `high`) |
| `antigravity-gemini-3.5-flash` | Yes | No | No | Active Antigravity quota set (`low`, `medium`, `high`) |
| `antigravity-gemini-3.1-pro` | Yes | No | No | Active Antigravity quota set (`low`, `high`) |
| `antigravity-gemini-3-pro` | Yes | No | No | Active Antigravity quota set (`low`, `high`) |
| `antigravity-gemini-3-flash` | Yes | No | No | Active Antigravity quota set (`minimal`, `low`, `medium`, `high`) |
| `antigravity-claude-sonnet-4-6` | Yes | No | No | Active Antigravity quota set (Standard non-thinking) |
| `antigravity-claude-opus-4-6-thinking` | Yes | No | No | Active Antigravity quota set (`low`, `max` thinking budgets) |
| `antigravity-gpt-oss-120b-medium` | Yes | No | No | Active Antigravity quota set |
| `antigravity-gemini-3.6-flash-high` | Yes | No | No | Legacy tier alias (resolves to `antigravity-gemini-3.6-flash` + `high`) |
| `antigravity-gemini-3.1-pro-low` | Yes | No | No | Legacy tier alias (resolves to `antigravity-gemini-3.1-pro` + `low`) |
| `antigravity-gemini-3.5-flash-medium` | Yes | No | No | Legacy tier alias (resolves to `antigravity-gemini-3.5-flash` + `medium`) |
