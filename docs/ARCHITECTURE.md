...
3. **Dual quota (Gemini)** - Antigravity + Gemini CLI headers (`headerStyle` quota enforcement & `isRateLimitedForHeaderStyle` hybrid selection check)
4. **Automatic failover & Circuit Breaker** - On 429, switch to next available account; includes 1s circuit breaker sleep during rate-limit cooldowns to prevent spin loops
5. **Automatic failover** - On 429, switch to next available account
